...
# ---------------------------------------------------------------------------
# CLAUDE.md — project rules injected into the working directory
# ---------------------------------------------------------------------------
_CLAUDE_MD_MARKER = "# [AIDevelop Bot Rules]"
...
- Shell commands (Bash) must only operate within this directory.

## Allowed Actions
- File operations (Read, Write, Edit, Glob, Grep) — within this folder only
- Shell commands (Bash) — within this folder only
- Web search and web fetch — allowed
"""


def ensure_claude_md(cwd: str) -> None:
    """Create or append bot rules to CLAUDE.md in the working directory."""
    ...
    if os.path.isfile(md_path):
        ...
        if _CLAUDE_MD_MARKER in content:
            print(f"  [CLAUDE.md] Rules already present in {md_path}")
            return
        # Append rules to existing file
        with open(md_path, "a", encoding="utf-8") as f:
            f.write("\n\n" + _CLAUDE_MD_RULES)
        print(f"  [CLAUDE.md] Appended rules to {md_path}")
    else:
        with open(md_path, "w", encoding="utf-8") as f:
            f.write(_CLAUDE_MD_RULES)
        print(f"  [CLAUDE.md] Created {md_path}")
...
async def on_ready():
    ...
    await bot.change_presence(
        activity=discord.Activity(
            ...
            name=f"{COMMAND_PREFIX}help | {tool['name']}",
        )
    )
